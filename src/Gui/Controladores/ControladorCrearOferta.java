...
public class ControladorCrearOferta implements ActionListener {
    ...
    public ControladorCrearOferta(SubpanelCrearOferta vista,
                                   Cliente cliente,
    ...
    /**
     * Devuelve los productos del cliente disponibles para ofertar.
     * Solo los que están tasados, visibles y no bloqueados.
     *
    ...
    /**
     * Envía la oferta con los productos seleccionados por el usuario.
     * Guarda el estado tras enviar correctamente.
     */
    private void enviarOferta() {
        ...
        if (misSeleccionados.isEmpty()) {
            vista.mostrarError(
                "Debes seleccionar al menos un producto tuyo para ofrecer.");
            return;
        }
        if (susSeleccionados.isEmpty()) {
            vista.mostrarError(
                "Debes seleccionar al menos un producto del otro usuario.");
            return;
        }

        try {
            Cliente destinatario = productoObjetivo.getPropietario();
            boolean ok = cliente.proponerOferta(
                destinatario, misSeleccionados, susSeleccionados);
            if (ok) {
                ...
                vista.mostrarError(
                    ...
                    "Comprueba que los productos estén tasados y disponibles.");
            }
        } catch (ProductoBloqueadoException ex) {
            vista.mostrarError(
                "Uno de los productos está bloqueado por otra oferta pendiente.");
        } catch (Exception ex) {
