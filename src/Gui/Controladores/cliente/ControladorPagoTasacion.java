...
public class ControladorPagoTasacion implements ActionListener {
    ...
    /**
     * Valida los campos y realiza el pago de la tasación.
    ...
    private void realizarPago() {
        ...
        boolean ok = cliente.solicitarTasacion(
            producto, numeroTarjeta, cvv, fechaTarjeta);
        if (ok) {
            GuardadoTienda.guardar(Tienda.getInstancia());
            vista.mostrarExito(
                "Tasación solicitada. Un empleado valorará "
                + "tu producto pronto.");
            vista.volverACartera();
        } else {
            vista.mostrarError(
                "No se pudo procesar el pago de la tasación.");
        }
    }
