 ...
 * Gestiona toda la lógica de autenticación y navegación desde el login.
 *
 * @author Daniel
 * @version 1.0
 */
public class ControladorLogin {
    ...
    public void realizarRegistro(String nickname, String password, String dni, String tipo) {
        ...
        try {
            if ("Cliente".equals(tipo)) {
                if (dni.isEmpty()) {
                    vista.mostrarError("El DNI es obligatorio para clientes.");
                    return;
                }
                ...
                if (nuevo != null) {
                    ...
                    if (logueado != null) {
                        vista.mostrarExito("¡Empleado registrado correctamente, " + nickname + "!");
                        ventana.loginEmpleado(logueado);
                    }
                } else {
                    vista.mostrarError(
                        "No se pudo crear la cuenta de empleado. Comprueba que:\n"
                        + "- El nickname no esté en uso\n"
                        + "- La contraseña sea segura");
                }
            }
