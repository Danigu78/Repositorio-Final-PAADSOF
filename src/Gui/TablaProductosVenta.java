...
public class TablaProductosVenta extends JPanel {
    ...
    private final ControladorProductosEmpleado controlador =
        new ControladorProductosEmpleado();
    ...
    public static class ColumnaExtra {
        ...
        public ColumnaExtra(String nombre, Function<ProductoVenta, Object> valor,
                String ordenAscendente, String ordenDescendente,
                Comparator<ProductoVenta> comparadorAscendente) {
            this.nombre = nombre;
            this.valor = valor;
    ...
    public TablaProductosVenta(Supplier<List<ProductoVenta>> proveedorProductos,
            List<ColumnaExtra> columnasExtra) {
    ...
    private void construir() {
        ...
        scroll.setPreferredSize(new Dimension(
            VentanaPrincipal.escalar(1050), VentanaPrincipal.escalar(240)));
    ...
    private JPanel crearPanelFiltros() {
        ...
        campoBuscar.setPreferredSize(new Dimension(
            VentanaPrincipal.escalar(650), VentanaPrincipal.escalar(34)));
        campoBuscar.setMaximumSize(new Dimension(
            VentanaPrincipal.escalar(650), VentanaPrincipal.escalar(34)));
        ...
        comboOrden.setPreferredSize(new Dimension(
            VentanaPrincipal.escalar(230), VentanaPrincipal.escalar(34)));
        comboOrden.setMaximumSize(new Dimension(
            VentanaPrincipal.escalar(230), VentanaPrincipal.escalar(34)));

        checksTipo = new JCheckBox[] {
            crearCheckFiltro("Comic"),
            crearCheckFiltro("Juego"),
            crearCheckFiltro("Figura"),
        ...
        botonLimpiar.setPreferredSize(new Dimension(
            VentanaPrincipal.escalar(140), VentanaPrincipal.escalar(34)));

        JPanel filaSuperior = new JPanel(new FlowLayout(
            FlowLayout.LEFT, VentanaPrincipal.escalar(18), 0));
        filaSuperior.setOpaque(false);
        filaSuperior.add(crearZona("Buscar", campoBuscar,
            VentanaPrincipal.escalar(650)));
        filaSuperior.add(crearZona("Ordenar por", comboOrden,
    ...
    private JPanel crearFilaFiltro(String titulo, JCheckBox[] checks) {
        ...
        etiqueta.setPreferredSize(new Dimension(
            VentanaPrincipal.escalar(80), VentanaPrincipal.escalar(28)));
    ...
    private void cargarModelo() {
        ...
        ordenarProductos(productos, comboOrden == null
            ? "Sin ordenar" : String.valueOf(comboOrden.getSelectedItem()));
    ...
    private boolean pasaFiltro(String id, String nombre, String tipo,
            String categorias, String texto, List<String> tiposBuscados,
            List<String> categoriasBuscadas) {
        if (!texto.isEmpty()
                && !contiene(id, texto)
                && !contiene(nombre, texto)
                && !contiene(tipo, texto)
                && !contiene(categorias, texto)) {
            return false;
        }

        if (!tiposBuscados.isEmpty()
                && !tiposBuscados.contains(tipo)) {
            return false;
        }
    ...
    private void ordenarProductos(List<ProductoVenta> productos, String orden) {
        Comparator<ProductoVenta> comparador = Comparator.comparingInt(
            p -> numeroIdProducto(p.getId()));

        if ("Nombre A-Z".equals(orden)) {
            comparador = Comparator.comparing(
                p -> textoSeguro(p.getNombre()), String.CASE_INSENSITIVE_ORDER);
        } else if ("Nombre Z-A".equals(orden)) {
            comparador = Comparator.comparing(
                (ProductoVenta p) -> textoSeguro(p.getNombre()),
            ...
            for (ColumnaExtra extra : columnasExtra) {
                if (extra.ordenAscendente != null
                        && extra.ordenAscendente.equals(orden)) {
                    comparador = extra.comparadorAscendente;
                    break;
                }
                if (extra.ordenDescendente != null
                        && extra.ordenDescendente.equals(orden)) {
    ...
    private List<String> obtenerNombresCategoriasVenta() {
        ...
        for (Categoria categoria : tienda.Tienda.getInstancia().getCategorias()) {
    ...
    private <T> JComboBox<T> crearCombo(T[] valores) {
        ...
        combo.setBorder(BorderFactory.createLineBorder(
            VentanaPrincipal.COLOR_BORDE, 1));
        return combo;
    }
