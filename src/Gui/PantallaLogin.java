...
/**
 * Pantalla de login y registro de CheckPoint. Extiende PanelBaseInterfaz para
 * reutilizar helpers visuales. Sigue el patrón MVC de los apuntes.
 *
 * @author Daniel
 * @version 1.0
 */
public class PantallaLogin extends PanelBaseInterfaz {
	...
	private void inicializarUI() {
		JSplitPane split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, crearPanelLogo(), crearPanelFormularios());
	...
	private JLabel crearLogo() {
		...
		if (url != null) {
			...
			Image img = icono.getImage().getScaledInstance(VentanaPrincipal.escalar(440), VentanaPrincipal.escalar(270),
					Image.SCALE_SMOOTH);
	...
	private JPanel crearFormularioLogin() {
		...
		comboTipoUsuario = crearCombo(new String[] { "Cliente", "Empleado", "Gestor" });
		gbc.gridy = 2;
		...
		botonIrRegistro = crearBotonLoginSecundario("¿No tienes cuenta? Regístrate");
		gbc.gridy = 9;
	...
	private JPanel crearFormularioRegistro() {
		...
		JComboBox<String> comboTipoRegistro = crearCombo(new String[] { "Cliente" });
		...
		// crearLabel() de PanelBaseInterfaz
		JLabel infoPassword = crearLabel("<html><small>La contraseña debe tener mayúsculas, minúsculas,"
		...
		botonIrLogin = crearBotonLoginSecundario("¿Ya tienes cuenta? Inicia sesión");
		gbc.gridy = 12;
	...
	public void hacerLogin() {
		controlador.realizarLogin(campoNickname.getText().trim(), new String(campoPassword.getPassword()),
				String.valueOf(comboTipoUsuario.getSelectedItem()));
	}

	public void hacerRegistro() {
		controlador.realizarRegistro(campoNicknameReg.getText().trim(), new String(campoPasswordReg.getPassword()),
				campoDNI.getText().trim().toUpperCase(), "Cliente");
	}
	...
	/**
	 * Botón principal del login — naranja con hover COLOR_ACENTO2. Distinto de
	 * crearBotonNaranja() porque usa COLOR_ACENTO2 en hover.
	 */
	...
	/**
	 * Campo de contraseña — único de login, no está en PanelBaseInterfaz.
	 */
