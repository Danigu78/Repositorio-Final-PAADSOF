...
/**
 * Subpanel para crear una oferta de intercambio.
 * Muestra dos columnas: tus productos disponibles y los del propietario.
 * El usuario selecciona con checkboxes qué productos incluir.
...
public class SubpanelCrearOferta extends JPanel {
     ...
     */
    public SubpanelCrearOferta(VentanaPrincipal ventana,
                                SubpanelProducto2Mano subpanelOrigen,
                                Producto2Mano productoObjetivo,
                                Cliente cliente) {
    ...
    private JPanel crearPanelOferta() {
        JPanel panel = new JPanel(
            new GridLayout(1, 2, VentanaPrincipal.escalar(20), 0));
        ...
        panel.add(crearColumnaProductos(
            "Mis productos para ofrecer",
            misProductos, checkboxesMios,
        ...
        panel.add(crearColumnaProductos(
            "Productos de " + productoObjetivo.getPropietario().getNickname(),
            susProductos, checkboxesSuyos,
            "Este usuario no tiene productos disponibles."));
     ...
     */
    private JPanel crearColumnaProductos(String titulo,
        ...
        if (productos.isEmpty()) {
            ...
            for (Producto2Mano p : productos) {
                // Fila con checkbox y precio
                JPanel fila = new JPanel(new BorderLayout(
                    VentanaPrincipal.escalar(10), 0));
                fila.setBackground(VentanaPrincipal.COLOR_PANEL);
                fila.setMaximumSize(new Dimension(
                    Integer.MAX_VALUE, VentanaPrincipal.escalar(40)));
                fila.setBorder(BorderFactory.createMatteBorder(
                    0, 0, 1, 0, VentanaPrincipal.COLOR_BORDE));

                // Checkbox con el nombre del producto
                JCheckBox checkbox = new JCheckBox(p.getNombre());
                ...
                if (p.getValoracion() != null) {
                    JLabel labelPrecio = new JLabel(String.format(
                        "%.2f€", p.getValoracion().getPrecioTasacion()));
    ...
    private JPanel crearPanelBotonEnviar() {
        ...
        botonEnviar.setBorder(BorderFactory.createEmptyBorder(
            VentanaPrincipal.escalar(12), VentanaPrincipal.escalar(30),
            VentanaPrincipal.escalar(12), VentanaPrincipal.escalar(30)));
     ...
     * Lo lee el controlador para enviar la oferta.
     *
    ...
    public void mostrarExito(String mensaje) {
        JOptionPane.showMessageDialog(this, mensaje, "Oferta enviada",
