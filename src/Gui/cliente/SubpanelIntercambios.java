...
/**
 * Subpanel de intercambios del cliente.
 * Muestra cinco pestañas: ofertas enviadas, recibidas, aceptadas pendientes
 * de confirmación por empleado, historial y rechazadas/caducadas.
...
public class SubpanelIntercambios extends AbstractPanelCliente {
     ...
     * Crea el controlador, registra los botones y rellena todas las secciones.
     *
    ...
    private JPanel crearBarraInterna() {
        ...
        barra.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(0, 0,
                VentanaPrincipal.escalar(2), 0, VentanaPrincipal.COLOR_ACENTO),
        ...
        JPanel zonaPestañas = new JPanel(
            new FlowLayout(FlowLayout.LEFT, VentanaPrincipal.escalar(8), 0));
        zonaPestañas.setBackground(VentanaPrincipal.COLOR_PANEL);
        zonaPestañas.setBorder(BorderFactory.createEmptyBorder(
            VentanaPrincipal.escalar(9), 0, 0, 0));
    ...
    /**
     * Crea un panel con scroll para una sección y guarda la referencia al contenido.
     *
    ...
    private JPanel crearPanelContenido(String seccion) {
        ...
        switch (seccion) {
            case "enviadas":   contenidoEnviadas   = contenido; break;
            case "recibidas":  contenidoRecibidas  = contenido; break;
            case "historial":  contenidoHistorial  = contenido; break;
            case "rechazadas": contenidoRechazadas = contenido; break;
            case "aceptadas":  contenidoAceptadas  = contenido; break;
        }
    ...
    public void mostrarSeccion(String seccion) {
        ...
        switch (seccion) {
            case "ENVIADAS":   marcarBotonActivo(botonEnviados);   break;
            case "RECIBIDAS":  marcarBotonActivo(botonRecibidos);  break;
            case "HISTORIAL":  marcarBotonActivo(botonHistorial);  break;
            case "RECHAZADAS": marcarBotonActivo(botonRechazadas); break;
            case "ACEPTADAS":  marcarBotonActivo(botonAceptadas);  break;
        }
    }

    /**
     * Aplica el estilo visual de activo al botón indicado
     * y quita el estilo al botón previamente activo.
     *
    ...
    private void marcarBotonActivo(JButton boton) {
        ...
        botonActivo.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(0, 0, linea, 0,
    ...
    private void rellenarEnviadas() {
        ...
        if (ofertas.isEmpty()) {
            ...
            contenidoEnviadas.add(crearLabelVacio(
                "No tienes ofertas enviadas pendientes."));
        } else {
            for (Oferta o : ofertas)
                // añadirTarjetaConSeparacion() de AbstractPanelCliente
                añadirTarjetaConSeparacion(contenidoEnviadas,
                    crearTarjetaOferta(o, false), 8);
        }
     ...
     * Solo muestra las PENDIENTE con botones de acción.
     */
    private void rellenarRecibidas() {
        ...
        for (Oferta o : ofertas) {
            if (o.getEstado() == EstadoOferta.PENDIENTE) {
                añadirTarjetaConSeparacion(contenidoRecibidas,
                    crearTarjetaOferta(o, true), 8);
                hayPendientes = true;
            }
        }
        if (!hayPendientes)
            contenidoRecibidas.add(crearLabelVacio(
    ...
    /**
     * Rellena el panel de ofertas aceptadas pendientes de confirmación por empleado.
     */
    private void rellenarAceptadas() {
        contenidoAceptadas.removeAll();
        List<Oferta> ofertas = controlador.getOfertasAceptadasPendientes();
        if (ofertas.isEmpty()) {
            contenidoAceptadas.add(crearLabelVacio(
                "No tienes intercambios pendientes de confirmación."));
        } else {
            for (Oferta o : ofertas)
                añadirTarjetaConSeparacion(contenidoAceptadas,
                    crearTarjetaAceptada(o), 8);
        }
    ...
    private void rellenarHistorial() {
        ...
        if (ofertas.isEmpty()) {
            contenidoHistorial.add(crearLabelVacio(
                "No tienes intercambios realizados todavía."));
        } else {
            for (Oferta o : ofertas)
                añadirTarjetaConSeparacion(contenidoHistorial,
                    crearTarjetaHistorial(o), 8);
        }
    ...
    private void rellenarRechazadas() {
        ...
        if (ofertas.isEmpty()) {
            contenidoRechazadas.add(crearLabelVacio(
                "No tienes ofertas rechazadas ni caducadas."));
        } else {
            for (Oferta o : ofertas)
                añadirTarjetaConSeparacion(contenidoRechazadas,
                    crearTarjetaRechazada(o), 8);
        }
     ...
     * Reutiliza crearPanelInfoTarjeta() y crearGbcTarjeta() de AbstractPanelCliente.
     *
    ...
    /**
     * Devuelve un GridBagConstraints preconfigurado para filas de tarjeta de oferta.
     *
     ...
     * Si es recibida muestra botones aceptar/rechazar.
     * Si es enviada muestra el estado y el tiempo restante si está PENDIENTE.
     *
    ...
    private JPanel crearTarjetaOferta(Oferta oferta, boolean esRecibida) {
        ...
        JLabel labelOfrece = crearLabel(
            "Ofrece: " + getListaProductos(oferta.getProductosOfertados()));
        labelOfrece.setFont(VentanaPrincipal.FUENTE_PEQUENA);
        gbc.gridy = 2;
        panelInfo.add(labelOfrece, gbc);

        JLabel labelSolicita = crearLabel(
            "Solicita: " + getListaProductos(oferta.getProductosSolicitados()));
        ...
        if (oferta.getEstado() == EstadoOferta.PENDIENTE) {
            JLabel labelTiempo = crearLabel(
                controlador.getTiempoRestante(oferta));
        ...
        if (esRecibida) {
            ...
            JLabel labelEstado = crearLabel(
                controlador.getTextoEstado(oferta));
    ...
    /**
     * Crea una tarjeta para una oferta aceptada pendiente de confirmación.
     * Muestra el estado en azul sin botones.
     *
    ...
    private JPanel crearTarjetaAceptada(Oferta oferta) {
        ...
        JLabel labelId = new JLabel(
            oferta.getId() + "  ⏳ Esperando confirmación de empleado");
        labelId.setFont(VentanaPrincipal.FUENTE_BOTON);
        ...
        String conQuien = soyOrigen
            ? "Con: " + oferta.getDestino().getNickname()
    ...
    /**
     * Crea una tarjeta para un intercambio del historial.
     * Muestra quién entregó y quién recibió según si el cliente es origen o destino.
     *
    ...
    private JPanel crearTarjetaHistorial(Oferta oferta) {
        ...
        JLabel labelId = new JLabel(
            oferta.getId() + "  —  " + controlador.getTextoEstado(oferta));
        labelId.setFont(VentanaPrincipal.FUENTE_BOTON);
        ...
        List<Producto2Mano> entregados = soyOrigen
            ? oferta.getProductosOfertados() : oferta.getProductosSolicitados();
        List<Producto2Mano> recibidos = soyOrigen
            ? oferta.getProductosSolicitados() : oferta.getProductosOfertados();

        JLabel labelEntregados = crearLabel(
            "Entregaste: " + getListaProductos(entregados));
        labelEntregados.setFont(VentanaPrincipal.FUENTE_PEQUENA);
        gbc.gridy = 2;
        panelInfo.add(labelEntregados, gbc);

        JLabel labelRecibidos = crearLabel(
            "Recibiste: " + getListaProductos(recibidos));
    ...
    /**
     * Crea una tarjeta para una oferta rechazada o caducada.
     * Muestra el motivo en rojo si fue rechazada o en amarillo si caducó.
     *
    ...
    private JPanel crearTarjetaRechazada(Oferta oferta) {
        ...
        String motivo = oferta.getEstado() == EstadoOferta.CADUCADA
            ? "Caducada" : "Rechazada";
        Color colorEstado = oferta.getEstado() == EstadoOferta.CADUCADA
            ? new Color(150, 100, 0) : new Color(180, 50, 50);
        ...
        JLabel labelOfrece = crearLabel(
            "Ofrecía: " + getListaProductos(oferta.getProductosOfertados()));
        labelOfrece.setFont(VentanaPrincipal.FUENTE_PEQUENA);
        gbc.gridy = 2;
        panelInfo.add(labelOfrece, gbc);

        JLabel labelSolicita = crearLabel(
            "Solicitaba: " + getListaProductos(oferta.getProductosSolicitados()));
        labelSolicita.setFont(VentanaPrincipal.FUENTE_PEQUENA);
        gbc.gridy = 3;
        panelInfo.add(labelSolicita, gbc);
    ...
    private String getListaProductos(List<Producto2Mano> productos) {
        if (productos == null || productos.isEmpty()) return "ninguno";
     ...
     * Tras aceptar navega a la pestaña de aceptadas.
     *
    ...
    public void procesarAceptarOferta(String idOferta) {
        ...
        if (ok) {
            mostrarMensaje(
                "Oferta aceptada. Un empleado confirmará el intercambio.");
            actualizar(cliente);
