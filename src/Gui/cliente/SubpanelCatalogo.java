...
public class SubpanelCatalogo extends AbstractPanelCliente {
    ...
    public void actualizar(Cliente cliente) {
        ...
        mostrarProductos(ordenarProductos(
            new ArrayList<>(controlador.obtenerTodosLosProductos())));
        mostrarRecomendados();
        cardLayout.show(panelContenido, "CATALOGO");
    }
        ...
        panelProductos = new JPanel(
        	...
        	scroll.getViewport().addComponentListener(new ComponentAdapter() {
        	    ...
        	    public void componentResized(ComponentEvent e) {
        	        ...
        	        int anchTarjeta = VentanaPrincipal.escalar(300)
        	            + VentanaPrincipal.escalar(25);
        	        ...
        	        int altTarjeta = VentanaPrincipal.escalar(350)
        	            + VentanaPrincipal.escalar(25);
        	        int altoTotal = filas * altTarjeta
        	            + VentanaPrincipal.escalar(20);
        	        panelProductos.setPreferredSize(
        	            new Dimension(anchoViewport, altoTotal));
     ...
     * Fila 1: búsqueda, categoría, precio, orden, buscar, limpiar.
     * Fila 2: checkboxes de tipo + contador.
     *
    ...
    private JPanel crearPanelBusqueda() {
        ...
        panel.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(0, 0,
                VentanaPrincipal.escalar(3), 0, VentanaPrincipal.COLOR_ACENTO),
        ...
        JPanel filaFiltros = new JPanel(
            new FlowLayout(FlowLayout.LEFT, VentanaPrincipal.escalar(12), 0));
        filaFiltros.setBackground(VentanaPrincipal.COLOR_PANEL);

        filaFiltros.add(crearIcono("/fotos/lupa.jpg",
        ...
        spinnerPrecioMin = new JSpinner(
            new SpinnerNumberModel(0.0, 0.0, 9999.0, 1.0));
        ...
        spinnerPrecioMax = new JSpinner(
            new SpinnerNumberModel(9999.0, 0.0, 9999.0, 1.0));
        ...
        comboOrden.setPreferredSize(new Dimension(
            VentanaPrincipal.escalar(200), VentanaPrincipal.escalar(30)));
        ...
        JPanel filaTipos = new JPanel(
            new FlowLayout(FlowLayout.LEFT, VentanaPrincipal.escalar(10), 0));
        filaTipos.setBackground(VentanaPrincipal.COLOR_PANEL);
        filaTipos.setBorder(BorderFactory.createEmptyBorder(
     ...
     * Aplica nombre, categoría, precio, tipo y orden.
     * El orden lo delega en el controlador.
     * Lo llama el controlador y el Enter del campo.
     */
    public void buscar() {
        ...
        List<ProductoVenta> productos = controlador.filtrarProductos(
    ...
    /**
     * Devuelve el tipo de un producto como String legible.
     * Solo se usa aquí para filtrar por tipo en la vista.
     *
     * @param p El producto
     * @return "Pack", "Juego", "Figura", "Comic" o "Otro"
     */
    private String obtenerTipoProducto(ProductoVenta p) {
        if (p instanceof Pack) return "Pack";
        String nombre = p.getClass().getSimpleName().toLowerCase();
        if (nombre.contains("juego"))  return "Juego";
        if (nombre.contains("figura")) return "Figura";
        if (nombre.contains("comic"))  return "Comic";
        return "Otro";
    }
    ...
    private List<ProductoVenta> ordenarProductos(List<ProductoVenta> productos) {
        if (controlador == null || comboOrden == null) return productos;
        String orden = (String) comboOrden.getSelectedItem();
        if (orden == null) return productos;
    ...
    public void resetearFiltros() {
        ...
        if (comboOrden  != null) comboOrden.setSelectedIndex(0);
        if (checkJuego  != null) checkJuego.setSelected(false);
        if (checkPack   != null) checkPack.setSelected(false);
        if (checkFigura != null) checkFigura.setSelected(false);
        if (checkComic  != null) checkComic.setSelected(false);
        mostrarProductos(controlador.ordenarPorId(
            new ArrayList<>(controlador.obtenerTodosLosProductos())));
        mostrarRecomendados();
    }
    ...
    private void mostrarRecomendados() {
        ...
        JPanel panelTarjetas = new JPanel(new FlowLayout(FlowLayout.LEFT,
            VentanaPrincipal.escalar(15), VentanaPrincipal.escalar(10)));
     ...
     * Si es Pack muestra badge y ahorro.
     *
    ...
    private JPanel crearTarjeta(ProductoVenta producto) {
        ...
        if (producto instanceof Pack) {
            ...
            panelInfo.add(badge);
            panelInfo.add(Box.createVerticalStrut(
                VentanaPrincipal.escalar(3)));
        }
    ...
    private JPanel crearTarjetaPequeña(ProductoVenta producto) {
        JPanel tarjeta = new JPanel(
            new BorderLayout(0, VentanaPrincipal.escalar(5)));
        tarjeta.setBackground(VentanaPrincipal.COLOR_TARJETA);
        tarjeta.setPreferredSize(new Dimension(
