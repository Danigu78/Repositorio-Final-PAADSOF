 ...
 * Reutiliza el mismo estilo de tarjetas que SubpanelCarrito.
 *
 * @author Daniel
 * @version 1.0
 */
public class SubpanelPedidos extends JPanel {
    ...
    private JPanel crearPanelLista() {
        ...
        titulo.setBorder(BorderFactory.createEmptyBorder(
            0, 0, VentanaPrincipal.escalar(15), 0));
    ...
    private JPanel crearTarjetaPedido(Pedido pedido) {
        ...
        gbc.insets = new Insets(
            VentanaPrincipal.escalar(2), 0,
            VentanaPrincipal.escalar(2), VentanaPrincipal.escalar(10));
        gbc.weightx = 1;
        ...
        if (controlador.estaPendientePago(pedido)) {
            ...
            JLabel labelTiempo = new JLabel(
                minutos <= 5
                    ? "⚠ Solo quedan " + minutos + " min para pagar"
                    : "Tiempo para pagar: " + minutos + " min");
            labelTiempo.setFont(VentanaPrincipal.FUENTE_PEQUENA);
            labelTiempo.setForeground(
                minutos <= 5 ? new Color(200, 50, 50) : VentanaPrincipal.COLOR_TEXTO2);
            gbc.gridy = 3;
        ...
        gbcB.insets = new Insets(
            VentanaPrincipal.escalar(3), 0, VentanaPrincipal.escalar(3), 0);
     ...
     * Reutiliza el mismo estilo de barra superior que SubpanelProducto.
     *
     * @param pedido El pedido a mostrar
     */
    private void verDetallePedido(Pedido pedido) {
        ...
        if (pedido.getDescuentoAplicado() != null) {
            JLabel labelDesc = new JLabel(
                "Descuento: " + pedido.getDescuentoAplicado().getNombre());
    ...
    /**
     * Crea una fila para un producto del pedido.
     * Mismo estilo que crearTarjetaProducto en SubpanelCarrito.
     *
    ...
    private JPanel crearFilaProducto(LineaPedido linea) {
        ...
        labelImagen.setPreferredSize(new Dimension(
            VentanaPrincipal.escalar(80), VentanaPrincipal.escalar(80)));
        cargarImagen(labelImagen, producto.getImagenRuta(),
            VentanaPrincipal.escalar(70), VentanaPrincipal.escalar(70));
        ...
        JLabel labelCantidad = new JLabel(
            "x" + linea.getCantidad() + "  —  "
            + String.format("%.2f€ / ud", linea.getPrecioVenta()));
    ...
    private Color getColorEstado(EstadoPedido estado) {
        switch (estado) {
            case PENDIENTE_PAGO:      return new Color(200, 150, 0);
            case PAGADO:              return new Color(50, 150, 50);
            case LISTO_PARA_RECOGER:  return new Color(0, 120, 200);
            case ENTREGADO:           return new Color(50, 180, 50);
            case CANCELADO:           return new Color(180, 50, 50);
            default:                  return VentanaPrincipal.COLOR_TEXTO2;
        }
    }
