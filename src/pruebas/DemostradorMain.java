...
public class DemostradorMain {
	...
	public static void main(String[] args) {
		...
		// Caso de error: producto ya en la categoria excepcion
		System.out.println("  Intentar añadir watchmen a Retro-Gaming (ya está en esa categoria):");
		boolean yaEnCategoria = empStock.añadirProductoACategoria(watchmen.getId(), "Retro-Gaming");
		System.out.println("  Resultado: " + (yaEnCategoria ? "OK" : "FALLIDO - producto ya estaba en la categoria"));
		...
		System.out.println("\n  Intentar añadir producto que ya esta en el pack (Catan):");
		boolean yaEnPack = empStock.añadirProductoaPack(catan.getId(), packGamer.getId(), 1);
		System.out.println("  Resultado: " + (yaEnPack ? "OK" : "FALLIDO - producto ya estaba en el pack"));

		System.out.println("\n  Intentar añadir producto con stock insuficiente al pack:");
		boolean stockInsuficiente = empStock.añadirProductoaPack(pandemic.getId(), packGamer.getId(), 999);
		System.out.println("  Resultado: " + (stockInsuficiente ? "OK" : "FALLIDO - stock insuficiente"));
		...
		System.out.println("Invitado busca utilizando filtros:");

		System.out.println("\n  Invitado filtra productos entre 10€ y 20€:");
		invitado.filtrarPorPrecio(10.0, 20.0);

		System.out.println("\n  Invitado filtra productos de categoria 'Familiar':");
		invitado.filtrarPorCategoria("Familiar");

		System.out.println("\n  Invitado filtra productos con puntuacion minima 7:");
		invitado.filtrarPorPuntuacion(7.0);

		System.out.println("\n  Invitado filtra Anime entre 10€ y 30€:");
		invitado.filtrarProductos(10.0, 30.0, 0.0, "Anime");

		System.out.println("\n  Invitado filtra entre 10€ y 50€ con puntuacion minima 8:");
		invitado.filtrarProductos(10.0, 50.0, 8.0);

		System.out.println("\n  Invitado filtra Anime entre 10€ y 20€ con puntuacion minima 7:");
		invitado.filtrarProductos(10.0, 20.0, 7.0, "Anime");

		System.out.println("\n  invitado filtra con puntuacion minima 10 (sin resultados):");
		invitado.filtrarPorPuntuacion(10.0);

		System.out.println("\n  Invitado filtra categoria inexistente:");
		invitado.filtrarPorCategoria("CategoriaFalsa");
		...
		System.out.println("  Alice intenta reseñar watchmen dos veces:");
		try {
			alice.escribirReseña(watchmen, 5, "Segunda reseña no permitida");
		...
		System.out.println("\n  Casos de error en la solicitud de tasacion:");
		...
		System.out.println("  Empleado intenta tasar con valor aproximado  negativo:");
		empTasador.tasarProducto(pAlice.getId(), -1.0, EstadoProducto.MUY_BUENO);
		...
		System.out.println("\n  Alice intenta ver su propia cartera:");
		alice.verCarteraCliente("alicia");

		System.out.println("\n  Alice ve la cartera de bob:");
		alice.verCarteraCliente("bob");

		System.out.println("\nINTERCAMBIOS: ");
		System.out.println("Alicia crea una oferta a bob.");
		...
		// PRIMERA PRUEBA: Oferta a sí misma
		System.out.println("  Alice intenta proponerse oferta a si misma:");
		try {
			// Alice se cambió el nombre a "alicia", asegúrate de que el objeto 'alice' sea
			// el correcto
		...
		// SEGUNDA PRUEBA: Productos bloqueados (Esta ya la tenías bien)
		...
		// Cerramos el scheduler para que no interfiera

		// ── Carrito olvidado ──────────────────────────────────────────────────────
		tienda.getComprobadorTiempos().cerrarGestorTiempo();
		try {
			Thread.sleep(5000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		tienda.getComprobadorTiempos().limpiarCarritos();
		if (carlos.getCarritoActual() != null) {
			carlos.getCarritoActual().vaciarCarrito();
		...
		carlos.añadirProductoCarrito(akira, 2);
		Carrito carritoOlvidado = carlos.getCarritoActual();
		if (carritoOlvidado != null) {
			carritoOlvidado.imprimirCarrito();
		}
		System.out.println("  Stock akira antes: " + akira.getStockDisponible());
		...
		System.out.println("  Carrito carlos: "
				+ (carlos.getCarritoActual() == null ? "VACIO - caducado correctamente" : "aun tiene productos"));
		...
		Producto2Mano pCarlosOfertaCaducada = carlos.getCarteraIntercambio()
				.get(carlos.getCarteraIntercambio().size() - 1);
		carlos.solicitarTasacion(pCarlosOfertaCaducada, "9999000011112222", 333, Date.valueOf("2031-12-01"));
		empTasador.tasarProducto(pCarlosOfertaCaducada.getId(), 8.0, EstadoProducto.MUY_BUENO);

		alice.subirProducto("Bleach Vol.1", "Como nuevo", "bleach.jpg");
		Producto2Mano pAliceOfertaCaducada = alice.getCarteraIntercambio()
				.get(alice.getCarteraIntercambio().size() - 1);
		alice.solicitarTasacion(pAliceOfertaCaducada, "1111222233334444", 111, Date.valueOf("2029-01-01"));
		empTasador.tasarProducto(pAliceOfertaCaducada.getId(), 10.0, EstadoProducto.MUY_BUENO);

		boolean ofertaCaducadaCreada = carlos.proponerOferta(alice,
				carlos.crearListaProductos2Mano(pCarlosOfertaCaducada),
				alice.crearListaProductos2Mano(pAliceOfertaCaducada));
		...
		// Apagar ComprobadorTiempos al finalizar
		tienda.getComprobadorTiempos().cerrarGestorTiempo();
		System.out.println("  ComprobadorTiempos apagado correctamente.");
		...
		try {
			rec.setPesos(1, 1, 1);
			System.out.println("  Recomendador activado | limite: " + rec.getLimiteMaximo() + " | pesos -> valoracion: "
					+ String.format("%.2f", rec.getPesoValoracion()) + " compras: "
					+ String.format("%.2f", rec.getPesoCompras()) + " categorias: "
					+ String.format("%.2f", rec.getPesoCategorias()));

			// Sugerencias para alice con pesos iguales
			System.out.println("\n  Sugerencias con pesos iguales (1,1,1):");
			...
			// Caso error - recomendador desactivado
			System.out.println("\n  Desactivar recomendador e intentar generar sugerencias:");
			...
			rec.setPesos(1, 1, 1);
			System.out.println("  Recomendador reactivado");
		...
		alice.verHistorialPedidos();
		System.out.println("  Cartera de alice ANTES del logout:");
		alice.verMiCartera();
		System.out.println("  Preferencias de alice ANTES del logout:");
		alice.verMisPreferencias();

		alice.logout();
		System.out.println("  Sesion activa alice: " + alice.isSesionIniciada());
		tienda.imprimirUsuariosConSesionActiva();

		// Alice vuelve a hacer login
		System.out.println("\n  Alice vuelve a hacer login:");
		alice = tienda.loginCliente("alicia", "Alicia@5678");
		System.out.println("  Sesion activa alice: " + alice.isSesionIniciada());

		System.out.println("  Historial de pedidos de alice TRAS el login (debe ser el mismo):");
		alice.verHistorialPedidos();
		System.out.println("  Cartera de alice TRAS el login (debe ser la misma):");
		alice.verMiCartera();
		...
		bob.logout();
		System.out.println("  Sesion activa bob: " + bob.isSesionIniciada());
		tienda.imprimirUsuariosConSesionActiva();

		// Bob vuelve a hacer login
		System.out.println("\n  Bob vuelve a hacer login:");
		...
		// Carlos intenta hacer login con contraseña incorrecta
		System.out.println("\n  Carlos intenta login con contraseña incorrecta:");
		...
		System.out.println("  Sesion activa carlos: " + carlos.isSesionIniciada());
		System.out.println("  Historial de pedidos de carlos TRAS el login:");
		carlos.verHistorialPedidos();
