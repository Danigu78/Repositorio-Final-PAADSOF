...
public class TiendaTest {
    ...
    static void guardarDat() throws Exception {
        ...
        if (fichero.exists()) {
            datOriginal = java.nio.file.Files.readAllBytes(
                fichero.toPath());
        }
    }
    ...
    void setUp() {
        ...
        gestor.darDeAltaEmpleados_Permisos(
            "emp_ges_test", "Stock@1234", permisosStock);
        Empleado emp = tienda.loginEmpleado("emp_ges_test", "Stock@1234");

        clienteA = tienda.registrarNuevoCliente(
            "alice", "Alice@1234", "11111111A");
        clienteB = tienda.registrarNuevoCliente(
            "bob", "Bob@1234", "22222222B");

        emp.añadirProducto_nuevo("C", "Watchmen_ges", "Comic test",
            "w.jpg", 15.0, 100,
            tienda.seleccionarCategorias("Anime_ges"),
            400, "DC", 1987, 0, 0, 0, null, null, 0, 0, 0, 0, null);
        emp.añadirProducto_nuevo("C", "Akira_ges", "Manga test",
            "a.jpg", 12.99, 100,
            tienda.seleccionarCategorias("Anime_ges"),
            350, "Kodansha", 1982, 0, 0, 0, null, null, 0, 0, 0, 0, null);
        emp.añadirProducto_nuevo("F", "Figura_ges", "Figura test",
            "f.jpg", 39.99, 100,
            tienda.seleccionarCategorias("Anime_ges"),
            0, null, 0, 20.0, 10.0, 8.0, "PVC", "Bandai",
            0, 0, 0, 0, null);
    ...
    void testBuscarClientePorIdCompleto() {
        ...
        Cliente nuevo = tienda.registrarNuevoCliente(
            "id_tester", "Pass@1234", "11112222K");
        assertNotNull(nuevo);
        assertEquals(nuevo, tienda.buscarCLientePorId(nuevo.getId()));

        assertNull(tienda.buscarCLientePorId(gestor.getId()),
            "No debe devolver al gestor aunque el ID coincida");
    }
    ...
    void testDescuentoVolumenCorrecto() {
        ...
        DescuentoVolumen desc = new DescuentoVolumen(
            "Promo Vol", inicio, fin, 100.0, 10.0);

        Carrito carrito = new Carrito(clienteA);
        carrito.añadirProducto(watchmen, 2);
        assertEquals(30.0, desc.aplicarDescuento(carrito));

        Carrito carritoCaro = new Carrito(clienteA);
        carritoCaro.añadirProducto(watchmen, 10);
    ...
    void testDescuentoCantidadCorrecto() {
        LocalDateTime inicio = LocalDateTime.now().minusDays(1);
        LocalDateTime fin    = LocalDateTime.now().plusDays(1);
        DescuentoCantidad desc = new DescuentoCantidad(
            "Promo Cant", inicio, fin, 5, 20.0);

        Carrito carrito = new Carrito(clienteB);
        carrito.añadirProducto(watchmen, 5);
        carrito.añadirProducto(akira, 1);
    ...
    void testDescuentoCategoriaCorrecto() {
        ...
        LocalDateTime inicio = LocalDateTime.now().minusDays(1);
        LocalDateTime fin    = LocalDateTime.now().plusDays(1);
        DescuentoCategoria desc = new DescuentoCategoria(
            "Promo Anime", inicio, fin, catAnime, 50.0);

        Carrito carrito = new Carrito(clienteA);
        carrito.añadirProducto(watchmen, 1);
    ...
    void testRegaloPromoCorrecto() {
        ...
        Carrito carrito = new Carrito(clienteB);
        carrito.añadirProducto(watchmen, 4);
    ...
    void testLogicaPackCompleta() {
        ...
        assertThrows(ProductoYaEnPackException.class,
            () -> pack.addProducto(watchmen, 1));
        assertThrows(ProductoInvalidoException.class,
            () -> pack.addProducto(pack, 1));
        assertThrows(ProductoInvalidoException.class,
            () -> pack.setPrecioOficial(29.5));
    ...
    void testModificarEliminarPack() {
        ...
        assertThrows(ProductoInvalidoException.class,
            () -> pack.modificarUnidades(watchmen, -1));
    ...
    void testNotificacionesTienda() {
        ...
        Notificacion n1 = new Notificacion(
            "Aviso Descuento", TipoNotificacion.DESCUENTO);
        @SuppressWarnings("unused")
        Notificacion n2 = new Notificacion(
            "Aviso Empleado", TipoNotificacion.EMPLEADOS);

        assertEquals(2, tienda.getHistorialNotificaciones().size(),
            "Debe haber 2 notificaciones en el historial");
        assertEquals(2, tienda.getNotificacionesNoLeidas().size(),
            "Ambas deben estar sin leer");

        assertTrue(tienda.getHistorialNotificaciones().contains(n1));
        n1.marcarComoLeida();
        assertEquals(1, tienda.getNotificacionesNoLeidas().size(),
            "Solo debe quedar 1 sin leer");

        assertEquals(1,
            tienda.getNotificacionesPorTipo(TipoNotificacion.DESCUENTO).size());
    }
    ...
    void testImpresionesYHistoriales() {
        ...
        Descuento d = new DescuentoVolumen(
            "Promo", hoy.minusDays(1), hoy.plusDays(1), 10.0, 10.0);
    ...
    void testImprimirHistorialDescuentos() {
        ...
        Descuento d = new DescuentoVolumen("Historial",
            LocalDateTime.now(), LocalDateTime.now().plusDays(1), 5.0, 5.0);
        tienda.getHistorialDescuentos().add(d);
        assertDoesNotThrow(() -> tienda.imprimirHistorialDescuentos());
    }
